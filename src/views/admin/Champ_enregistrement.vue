<template>
  ...
  <v-app :style="{background: $vuetify.theme.themes.light.background}">
    <v-toolbar
      ...
      max-height="100"
    ><v-spacer></v-spacer>
      <h2 class="mt-6" style="text-align: center;"> Manage your personal Timesheet on {{ getDate.mois }}</h2>
            ...
            <v-card class="ml-5 text-center mt-5 pa-7" max-width="900" >
              <div id="app" >
                  <v-main class="container align-center px-1" >
                    ...
                    <v-spacer></v-spacer>
                    <v-card>
                      <v-data-table :headers="headers" :items="filteredItems" :search="date" class="elevation-0">
                        ...
                        <template v-slot:item.Date="{ item }">
                          <div class="text-truncate" style="width: 100px">{{item.Date}}</div>
                        </template>
                        <template v-slot:item.Nom_projet="{ item }">
                          <div class="text-truncate" style="width: 100px">{{item.Nom_projet}}</div>
                        </template>
                        <template v-slot:item.Heure="{ item }">
                          <div class="text-truncate" style="width: 80px">{{item.Heure}}</div>
                        </template>
                      </v-data-table>
                      <!-- delete dialog -->
...
computed: {
  ...
  return this.effectuer.filter(item => {
    ...
    const monthYearFilter = this.getDate.date.split("-")
...
methods: {
  ...
  async onClickButton() {
      await this.$router.push('/recapitulation_mois');
    },
  // Get All Projets
  async get_all_projets() {
    let Numero_matricule = this.getUserProfile.Numero_matricule
      let response = await axios.get(`http://localhost:5000/projet_by_mat/${Numero_matricule}`);
